  ...
  *deletePropertyEvaluator(name: string): EvaluationGenerator<boolean> {
  ...
  toJs(): unknown {
    if (!this._cachedJsObject) {
      this._cachedJsObject = createStaticJsObjectLikeProxy(this);
    }
...
function createStaticJsObjectLikeProxy(obj: StaticJsObjectLike): unknown {
  ...
  const getOwnPropertyDescriptor = (propertyName: string) => {
    ...
    if (existingDef && !existingDef.configurable) {
      ...
      if (isStaticJsDataPropertyDescriptor(descriptor) && descriptor.writable) {
        target[propertyName] = obj.getProperty(propertyName).toJs();
