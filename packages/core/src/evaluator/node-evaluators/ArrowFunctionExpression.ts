...
import { instantiateArrowFunctionExpression } from "#algorithms/instantiate-arrow-function-expression.js";
...
function* arrowFunctionExpressionNodeEvaluator(node: ArrowFunctionExpression): EvaluationGenerator {
  ...
  const func = yield* instantiateArrowFunctionExpression(node, name, lexicalEnv, privateEnv);
  return func;
}
