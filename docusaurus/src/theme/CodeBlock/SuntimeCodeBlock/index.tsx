...
export default function SuntimeCodeBlock(props: Props): ReactNode {
  ...
  function handleRun() {
    ...
    const addLine = (line: OutputLine) => {
      if (mountedRef.current) setLines(prev => [...prev, line]);
    };
    ...
    realm.global.defineOwnPropertySync('console', {
      value: realm.types.toStaticJsValue({
        ...
        error: (...args: unknown[]) => addLine({ kind: 'error', text: args.map(serialize).join(' ') }),
    ...
    const runner = createTimeSharingTaskRunner({ operationsPerIteration: 10000, yieldTime: 50 });
